...
class RRT:
    def __init__(self, start, goal, obstacle_list, width, height, depth):
        ...
        self.kdtree_start = KDTree([[self.start.x, self.start.y, self.start.z]])
        self.kdtree_goal = KDTree([[self.goal.x, self.goal.y, self.goal.z]])
    ...
    def plan(self, max_iter=200):  #duplicate part based on the RRTtest
        for i in range(max_iter):
            if i % 2==0:
                ...
                nearest_node = self.get_nearest_node(self.kdtree_start,rnd_node)
                new_node = self.extend(nearest_node, rnd_node)

                if self.check_collision(new_node, nearest_node):
                    self.node_list.append(new_node)
                    self.kdtree_start = KDTree([[node.x, node.y, node.z] for node in self.node_list])
                    if self.calc_dist_to_goal(self.node_list[-1].x, self.node_list[-1].y, self.node_list[-1].z) <= 1:
                        return self.generate_final_course(len(self.node_list) - 1)
            else:
                rnd_node = self.get_random_node()
                nearest_node = self.get_nearest_node(self.kdtree_goal,rnd_node)
                new_node = self.extend(nearest_node, rnd_node)
                if self.check_collision(new_node, nearest_node):
                    self.node_list_goal.append(new_node)
                    self.kdtree_goal = KDTree([[node.x, node.y, node.z] for node in self.node_list_goal])
                    if self.calc_dist_to_goal(self.node_list_goal[-1].x, self.node_list_goal[-1].y, self.node_list_goal[-1].z) <= 1:
                        return self.generate_final_course(len(self.node_list_goal) - 1)
    ...
    def get_nearest_node(self, kdtree, node):  #compare with the RRTtest, add the kdtree to divide the start direction and goal direction
        dist, ind = kdtree.query([[node.x, node.y, node.z]], k=1)
        if kdtree == self.kdtree_start:
            return self.node_list[ind[0][0]]
        else:
            return self.node_list_goal[ind[0][0]]
